  ...
  const captureAndSearch = async () => {
    ...
    try {
      ...
      if (data.matches && data.matches.length > 0) {
        setPhotos(data.matches.map((url: string, index: number) => ({
    ...
    <div className="min-h-screen bg-[#0F1722] text-white font-sans flex flex-col relative overflow-hidden">
      ...
      <main className="flex-1 p-4 md:p-8 relative z-10 max-w-7xl mx-auto w-full">
            ...
            <div className="relative grid gap-5 p-5 sm:p-7 md:grid-cols-[1fr_auto] md:items-end">
              <div>
                ...
                <div className="mt-4 flex flex-wrap gap-3 text-[11px] font-bold text-blue-100/65">
                  ...
                  <span className="flex items-center gap-1.5"><Images className="h-3.5 w-3.5 text-orange-300" /> {activeTour.media_count || photos.length} lembranças</span>
        ...
        <div className="mb-6 flex flex-col justify-between gap-4 rounded-3xl border border-white/10 bg-white/[0.04] p-4 sm:flex-row sm:items-center">
          <div>
            <h3 className="text-xl font-black">{isFaceSearchMode ? 'Encontramos você!' : 'Todas as lembranças'}</h3>
            <p className="mt-1 text-xs text-gray-400">{selectionMode ? `${selectedPhotoIds.size} selecionada(s)` : `${photos.length} arquivo(s) em qualidade original`}</p>
          </div>
          <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
            ...
            <button onClick={openScanner} disabled={!selectedTour} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-purple-600 px-4 text-xs font-black shadow-[0_0_20px_rgba(147,51,234,0.25)] disabled:opacity-50"><Sparkles className="h-4 w-4" /> Achar minhas fotos</button>
        ...
        {loadingPhotos ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 text-gray-500 border-2 border-dashed border-white/10 rounded-3xl bg-white/5">
            <Camera className="w-12 h-12 mb-4 opacity-50" />
            <p className="font-bold text-lg text-white">{tours.length ? 'Nenhuma foto encontrada' : 'Nenhum álbum liberado ainda'}</p>
            <p className="text-sm mt-1 text-center px-5">
              {tours.length
                ? 'Aguarde a equipe enviar as fotos desta trilha.'
                : 'Quando a equipe publicar as fotos de uma trilha que você comprou, o álbum aparecerá aqui automaticamente.'}
            </p>
          </div>
        ) : (
          <motion.div 
